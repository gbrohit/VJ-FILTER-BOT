...
from info import (
    OWNER_LNK, REACTIONS, CHANNELS, ADMINS, SHORTLINK_MODE, 
    PREMIUM_AND_REFERAL_MODE, AUTH_CHANNEL, REFERAL_PREMEIUM_TIME, 
    REFERAL_COUNT, PAYMENT_TEXT, PAYMENT_QR, LOG_CHANNEL, PICS, 
    CUSTOM_FILE_CAPTION, PROTECT_CONTENT, CHNL_LNK, GRP_LNK, 
    REQST_CHANNEL, SUPPORT_CHAT, MAX_B_TN, VERIFY, SHORTLINK_API, 
    SHORTLINK_URL, TUTORIAL, VERIFY_TUTORIAL, IS_TUTORIAL
)
from utils import (
    ...
    if settings is not None:
        buttons = [
            ...
            [
                InlineKeyboardButton(
                    'Nᴏ-Rᴇsᴜʟᴛ Msɢ',
                    callback_data=f'setgs#no_result_msg#{settings.get("no_result_msg", True)}#{grp_id}',
                ),
                InlineKeyboardButton(
                    '✔ Oɴ' if settings.get("no_result_msg", True) else '✘ Oғғ',
                    callback_data=f'setgs#no_result_msg#{settings.get("no_result_msg", True)}#{grp_id}',
                ),
            ],
        ]
